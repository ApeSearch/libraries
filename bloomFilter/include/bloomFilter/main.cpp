...
int main(int argc, char** argv) {
    ...
    Bloomfilter bf(stoi(size), stod(error), "bloomfilter.bin");
    ...
    uint64_t counter = 0;
    while(fin >> input) {
        //bf.insert(APESEARCH::string( input.begin(), input.end() ));
        counter++;
    }
    ...
    while (true) {
        cin >> input;
        if (input == "q") return 0;
        bf.contains(APESEARCH::string( input.begin(), input.end() ) ) ? cout << "Found!\n" : cout << "Not found.\n";
    }
